/**
 * Minecraft Bot (mineflayer) 1.21+ with Auto-Eat, Auto-Respawn & Web Dashboard
 * ------------------------------------------------
 * - Connects to a Minecraft Java Edition server (1.21+) via mineflayer.
 * - Auto-eats from hotbar when hunger/health is low.
 * - Auto-respawns on death.
 * - Listens to in-game chat and runs commands with customizable prefix.
 * - Anti-AFK: looks around / hops every few seconds.
 * - Auto-reconnect on disconnect/kick/error.
 * - Join sequence: /register on first join, /login afterwards, then custom commands.
 * - Express + Socket.IO dashboard on PORT (default 3000) for 24/7 hosting.
 *
...
const AUTO_EAT = config.autoEat || { enabled: true, foodThresholdHP: 15, checkIntervalMs: 1000 };
const AUTO_RESPAWN = config.autoRespawn || { enabled: true };
...
io.on('connection', (socket) => {
  ...
  // Allow chat/command input from the web dashboard
  socket.on('send_chat', (message) => {
    if (bot && bot.chat) {
      log(`[Web Dashboard] Sending: ${message}`);
      bot.chat(message);
    }
  });
});
...
function createBot() {
  ...
  const newBot = mineflayer.createBot({
    ...
    version: config.version || false, // false => auto-detect; works for 1.21+
  });
  ...
  // -------------------------------------------------------------------------
  if (AUTO_RESPAWN.enabled) {
    newBot.on('death', () => {
      log('Bot died! Auto-respawning...');
      setTimeout(() => {
        if (newBot && newBot.respawn) {
          newBot.respawn();
          log('Respawned!');
        }
      }, 500);
...
// ---------------------------------------------------------------------------
// Auto-Eat: checks hunger/health and eats from hotbar when below threshold
// ---------------------------------------------------------------------------
function startAutoEat(bot) {
  if (!AUTO_EAT.enabled) return;
  const threshold = Number(AUTO_EAT.foodThresholdHP) || 15;
  const checkInterval = Number(AUTO_EAT.checkIntervalMs) || 1000;
  ...
  function tick() {
    ...
    if (health < threshold || foodLevel < threshold) {
      // Find food in hotbar (slots 0-8)
      let foodItem = null;
      for (let i = 0; i < 9; i++) {
        const item = bot.inventory.slots[i];
        if (item && isFoodItem(item.name)) {
          foodItem = item;
          break;
        }
      }
      ...
      if (foodItem) {
        log(`Auto-eating ${foodItem.name} (health: ${health}, food: ${foodLevel})`);
        bot.equip(foodItem, 'hand');
        bot.activate();
        // Hold eat for ~1 second
        setTimeout(() => {
          if (bot) bot.deactivate();
        }, 1000);
      }
    }

    autoEatTimer = setTimeout(tick, checkInterval);
  }
...
function isFoodItem(itemName) {
  const foodItems = [
    ...
    'beef', 'cooked_beef', 'pork', 'cooked_porkchop', 'mutton', 'cooked_mutton',
    'chicken', 'cooked_chicken', 'rabbit', 'cooked_rabbit',
    'cod', 'salmon', 'tropical_fish', 'pufferfish', 'cooked_cod', 'cooked_salmon',
    'bread', 'baked_potato', 'potato', 'poisonous_potato',
    'carrot', 'golden_carrot', 'pumpkin_pie', 'melon', 'melon_slice', 'glow_berries',
    'sweet_berries', 'chorus_fruit', 'cookie', 'cake',
  ];
  return foodItems.some(f => itemName && itemName.includes(f));
}
...
// configured "afterLoginCommands" (e.g. /warp afk).
// ---------------------------------------------------------------------------
...
// Post-spawn phase — runs once we're actually in the world (so /warp,
// commands etc. are valid and don't get swallowed by AuthMe).
...
function cleanup() {
  ...
  if (autoEatTimer) { clearTimeout(autoEatTimer); autoEatTimer = null; }
  isFollowing = false;
  followTarget = null;
}
